...
export async function GET(request: NextRequest) {
    ...
    try {
        const res = await fetch(`${NGINX_API}/cluster/admin/enforce-leader`, {
            headers: {
        ...
        if (!res.ok) {
            throw new Error(`Failed to enforce leader: ${res.statusText}`);
        }

        const data = await res.json();
        return NextResponse.json(data);
    } catch (error) {
        console.error('Error enforcing leader:', error);
        return NextResponse.json(
            {error: 'Failed to enforce leader'},
            {status: 500}
        );
    }
}
